            ...
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    <p>Weight: <span>{product.weight?.size || "N/A"}</span></p>
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select
                                            className='outline-none ml-2'
                                            value={product.quantity}
                                            onChange={(e) => updateCart(product._id, parseInt(e.target.value))}
                                        >
                        ...
                        <p className="text-center">{curreny}{product.offerPrice * product.quantity}</p>
                ...
                <button className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img onClick={() => { navigate('/products'); scrollTo(0, 0) }}
                        className="group-hover:translate-x-1 transition"
                        src={assets.arrow_right_icon_colored} alt="" />
                    <p onClick={() => { navigate('/products'); scrollTo(0, 0) }}>Continue Shopping</p>
                </button>
            </div>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    <p className="flex justify-between">
                        <span>Price</span><span>{curreny}{getCartAmount()}</span>
                    </p>
                    <p className="flex justify-between">
                        <span>Shipping Fee</span><span className="text-green-600">Free</span>
                    </p>
                    <p className="flex justify-between">
                        <span>Tax (2%)</span><span>{curreny}{(getCartAmount() * 0.02).toFixed(2)}</span>
                    </p>
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>Total Amount:</span><span>{curreny}{(getCartAmount() * 1.02).toFixed(2)}</span>
                    </p>
                </div>
